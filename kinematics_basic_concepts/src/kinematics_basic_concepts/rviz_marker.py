...
class MarkerBasics(object):
    ...
    def update_pose(self, x, y, roll, index):
        ...
        q = euler_to_quaternion(roll=roll, pitch=0.0, yaw=0.0)
    ...
    def publish_point(self,x, y, roll, index):
        """
        here Yaw will be the only one becuase it referes to he Z rotation
        """
        self.update_pose(x, y, roll, index)
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            self.publish_point(cos(theta), sin(theta), roll=theta, index=index)
            theta += 0.1
